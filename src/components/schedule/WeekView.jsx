...
import { formatColumnHeading, formatShortDate, isSameDay } from '../../data/schedule';

/**
 * Day-by-day board for one Monday-to-Sunday week. `columns` is one entry per
 * day, already bucketed by whichever source the schedule screen is reading.
 */
export const WeekView = ({ columns = [], today, onCreateJob }) => (
    ...
    {columns.map((column) => (
      ...
      >
        ...
        <div className="schedule-week__jobs">
          {column.jobs.map((job) => (
            <ScheduleJobCard job={job} key={job.assignmentId ?? job.id} draggable />
          ))}
